...
const LocationMap: React.FC<LocationMapProps> = ({ locations, selectedLocationId, onMarkerClick }) => {
  ...
  const updateMarkers = useCallback(() => {
    ...
    if (selectedLocationId && markersRef.current[selectedLocationId]) {
        googleMapRef.current.panTo(markersRef.current[selectedLocationId].getPosition());
        googleMapRef.current.setZoom(15);
    } else if (hasMarkers && !selectedLocationId) {
        googleMapRef.current.fitBounds(bounds);
    } else if (!hasMarkers) {
        googleMapRef.current.setCenter({ lat: TIMBERCREEK_CHURCH_COORDS.latitude, lng: TIMBERCREEK_CHURCH_COORDS.longitude });
  ...
  useEffect(() => {
    ...
    if (googleMapRef.current) { updateMarkers(); return; }

    try {
      googleMapRef.current = new window.google.maps.Map(mapRef.current, {
        center: { lat: TIMBERCREEK_CHURCH_COORDS.latitude, lng: TIMBERCREEK_CHURCH_COORDS.longitude },
        zoom: 12,
        ...
        mapId: 'DEMO_MAP_ID',
        styles: MAP_STYLES // Apply Dark Theme
      });
      updateMarkers();
  ...
  if (error) {
    return <div className="w-full h-full bg-surfaceHighlight border border-red-900 flex items-center justify-center text-red-500 font-bold uppercase">MAP OFFLINE</div>;
  }
