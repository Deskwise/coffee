...
const AnnouncementCard = React.lazy(
  () => import("./components/AnnouncementCard"),
);
const LocationMap = React.lazy(() => import("./components/LocationMap"));
// Input and Button are small, keep them sync
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkApiKey = async () => {
      if (
        ...
        if (!hasKey) {
          console.warn(
            "Gemini API key not selected. Some features might be limited.",
          );
        }
      } else {
  ...
  const handleHardReset = () => {
    if (
      confirm(
        "EXTREME DANGER: This will wipe all local data and reset the app to a fresh install state. This will fix map pins. Proceed?",
      )
    ) {
      localStorage.clear();
  ...
  // --- Timeslot Actions ---
  const handleCreateTimeslot = useCallback(
    async (newTimeslotData: Omit<Timeslot, "id" | "isBooked">) => {
      if (!currentUser) return;
      try {
        const createdTimeslot = await apiService.createTimeslot(
          newTimeslotData,
          currentUser.id,
        );
  ...
  const handleAcceptTimeslot = useCallback(
    async (timeslotId: string) => {
      ...
      try {
        ...
        const createdMeeting = await apiService.createMeeting(
          newMeeting,
          timeslotToBook.hostUserId,
          currentUser.id,
        );
        ...
        const location = locations.find(
          (l) => l.id === timeslotToBook.locationId,
        );
        ...
        if (host && attendee && location) {
          ...
          apiService.sendSMSNotification(
            `+1555${host.id.slice(-4)}`,
            hostMessage,
          );
          apiService.sendSMSNotification(
            `+1555${attendee.id.slice(-4)}`,
            attendeeMessage,
          );

          const icsContent = apiService.generateICSFile(
            createdMeeting,
            location,
            host,
            attendee,
        ...
        if (timeslot.isBooked && currentUser.role === UserRole.ADMINISTRATOR) {
          ...
          if (meeting) {
            ...
            if (host && attendee && location) {
              apiService.sendSMSNotification(
                `+1555${host.id.slice(-4)}`,
                `ALERT: Your meeting with ${attendee.name} on ${format(meeting.startTime, "MMM dd, yyyy h:mm a")} at ${location.name} has been cancelled by an administrator.`,
              );
              apiService.sendSMSNotification(
                `+1555${attendee.id.slice(-4)}`,
                `ALERT: Your meeting with ${host.name} on ${format(meeting.startTime, "MMM dd, yyyy h:mm a")} at ${location.name} has been cancelled by an administrator.`,
              );
            }
          }
        }
  ...
  const handleCancelMeeting = useCallback(
    async (meetingId: string, cancellingUserId: string) => {
      if (!currentUser) return;
      try {
        const cancelledMeeting = await apiService.cancelMeeting(
          meetingId,
          cancellingUserId,
        );
        setMeetings(await apiService.getMeetings());
        ...
        const attendee = users.find(
          (u) => u.id === cancelledMeeting.attendeeUserId,
        );
        const location = locations.find(
          (l) => l.id === cancelledMeeting.locationId,
        );
  ...
  const handleDeleteLocation = async (locationId: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this location? All associated timeslots and meetings will also be deleted.",
      )
    )
      return;
    try {
  ...
  const renderAdminDashboard = () => {
    ...
    return (
      <div className="p-4 pt-16 pb-20 max-w-5xl mx-auto">
        <h2 className="text-3xl font-bold text-center text-text mb-8 tracking-tight">
          Admin Dashboard
        </h2>
          ...
          {adminSubView === "Users" && (
            <div className="card-industrial p-6 rounded-xl border border-slate-700/50">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-text text-glow-amber">
                  Manage Users
                </h3>
                ...
                <table className="min-w-full divide-y divide-slate-700/50">
                  ...
                  <tbody className="divide-y divide-slate-700/50">
                    {users.map((user) => (
                      <tr
                        key={user.id}
                        className="hover:bg-surfaceHighlight/20 transition-colors"
                      >
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-text flex items-center">
                          <img
                            src={user.profilePicture}
                            alt={user.name}
                            className="w-8 h-8 rounded-full mr-3 object-cover border border-slate-600"
                          />
            ...
            <div className="card-industrial p-6 rounded-xl border border-slate-700/50">
              ...
              <div className="space-y-3 mb-8">
                {locations.filter((loc) => !loc.isApproved).length === 0 ? (
                  <div className="p-4 bg-surfaceHighlight/30 rounded-lg text-text-secondary text-sm italic border border-dashed border-slate-600 text-center">
                    No pending locations.
                  </div>
                ) : (
                  locations
                    .filter((loc) => !loc.isApproved)
                    .map((loc) => (
                      <div
                        key={loc.id}
                        className="bg-surfaceHighlight/20 p-4 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between shadow-sm border border-slate-700"
                      >
                        <div className="mb-3 sm:mb-0">
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Submitted by:{" "}
                            {users.find((u) => u.id === loc.submittedByUserId)
                              ?.name || "Unknown"}
                          </p>
                        </div>
                ...
                {locations.filter((loc) => loc.isApproved).length === 0 ? (
                  ...
                  locations
                    ...
                    .map((loc) => (
                      <div
                        key={loc.id}
                        className="bg-surfaceHighlight/20 p-4 rounded-lg flex items-center justify-between shadow-sm border border-slate-700"
                      >
                        <div>
                          <h5 className="font-semibold text-text">
                            {loc.name}
                ...
                {announcements.length === 0 ? (
                  ...
                  announcements.map((announcement) => (
                    <AnnouncementCard
                      ...
                      author={users.find(
                        (u) => u.id === announcement.authorUserId,
                      )}
  ...
  return (
    <div className="min-h-screen bg-background text-text selection:bg-primary selection:text-white overflow-x-hidden relative">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        {/* Floating Coffee Steam Particles - REDUCED COUNT FOR PERFORMANCE */}
        {[...Array(5)].map((_, i) => (
          <div
            key={`steam-${i}`}
        ...
        {/* COFFEE GLOW ORBS - Warm, Industrial - STATIC TRANSFORMS ARE BETTER THAN ANIMATIONS */}
        {/* Deep Espresso/Brown - Top Left (dark roast) */}
        <div
          className="absolute -top-10 -left-20 w-[750px] h-[750px] rounded-full blur-3xl opacity-60"
          style={{
            background:
              "radial-gradient(circle, rgba(92, 51, 23, 0.85) 0%, rgba(64, 35, 15, 0.6) 40%, rgba(42, 24, 10, 0.3) 70%, transparent 100%)",
          }}
        ></div>
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle, rgba(217, 119, 6, 0.7) 0%, rgba(180, 83, 9, 0.5) 40%, rgba(120, 53, 15, 0.3) 70%, transparent 100%)",
          }}
        ></div>

        {/* Burnt Orange - Bottom Left (whiskey, leather) */}
        <div
          className="absolute bottom-0 left-0 w-[700px] h-[700px] rounded-full blur-3xl opacity-50"
          style={{
            background:
              "radial-gradient(circle, rgba(234, 88, 12, 0.65) 0%, rgba(194, 65, 12, 0.45) 40%, rgba(154, 52, 18, 0.25) 70%, transparent 100%)",
          }}
        ></div>

        {/* Dark Forest Green/Charcoal - Bottom Right (natural, earthy) */}
        <div
          className="absolute bottom-0 right-0 w-[650px] h-[650px] rounded-full blur-3xl opacity-50"
          style={{
            background:
              "radial-gradient(circle, rgba(20, 83, 45, 0.55) 0%, rgba(15, 60, 35, 0.35) 40%, rgba(10, 40, 25, 0.2) 70%, transparent 100%)",
          }}
        ></div>

        {/* Center Warm Copper Glow - Industrial coffee machine vibes */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[900px] rounded-full blur-3xl opacity-30"
          style={{
            background:
              "radial-gradient(circle, rgba(161, 98, 7, 0.4) 0%, rgba(120, 53, 15, 0.25) 50%, rgba(80, 35, 10, 0.15) 80%, transparent 100%)",
          }}
        ></div>
      ...
      <Header
        ...
        onLogoClick={() => setCurrentView("Calendar")}
      />
        ...
        <Suspense
          fallback={
            <div className="flex flex-col items-center justify-center min-h-[50vh] text-primary">
              ...
              <p className="uppercase tracking-widest text-xs font-bold text-amber-500/80">
                Loading View...
              </p>
            </div>
          }
      ...
      >
        <div className="space-y-4">
          ...
          <Suspense
            fallback={
              <div className="h-48 bg-gray-900 animate-pulse rounded-lg" />
            }
          >
            <LocationMap
              locations={[
                {
                  ...
                  latitude:
                    typeof newLocationLat === "number"
                      ? newLocationLat
                      : 39.0069,
                  longitude:
                    typeof newLocationLon === "number"
                      ? newLocationLon
                      : -104.8818,
                  isApproved: false,
                },
